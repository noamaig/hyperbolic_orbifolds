        ...
        function [O,G,w]=objective_and_grad_fast(obj,X)
            assert(size(X,2)==1);
            assert(~any(isnan(X(:))));
            ...
            %karcher
            interior_length=length(obj.Winterior)*4;
            right_length=length(obj.Wright)*4;
            I=obj.Iinterior;
            J=obj.Jinterior;
            ...
            c=X(J*2-1);
            d=X(J*2);
            [V_k,O]=obj.karcher_grad_fast(a,b,c,d,obj.Winterior);
            V_k = [V_k; zeros(right_length,1)];
            
            w=obj.Winterior;
            
            
            
            J_k = [I*2-1; I*2; J*2-1; J*2; zeros(right_length,1)];
...
%             V_k=[grada;gradb;gradc;gradd];
            ...
            c=Y(:,1);
            d=Y(:,2);
            [Vr,o]=obj.karcher_grad_fast(a,b,c,d,obj.Wright);
            w=[w;obj.Wright];
            O=[O;o];
            ...
            c=X(right_ind_J*2-1);
            d=X(right_ind_J*2);
            Vl=obj.karcher_grad_fast(a,b,c,d,obj.Wright);
            %             O=[O;o];
            %add gradient to vector
            V_k(interior_length+1:end)=[Vr(1:(length(Vr)/2)) Vl((length(Vl)/2+1):end)];
            ...
            J_k(interior_length+1:end)=[Ja;Jb;Jc;Jd];
            if ~isempty(obj.IRightToRight)
                ...
                c=Y_I(:,1);
                d=Y_I(:,2);
                [grada,gradb,~,~,o]=obj.karcher_grad_fast(a,b,c,d,obj.WRightToRight);
                ...
                c=X(Jc);
                d=X(Jd);
                [~,~,gradc,gradd]=obj.karcher_grad_fast(a,b,c,d,obj.WRightToRight);
                %                 O=[O;o];
                %add gradient to vector
            ...
            if isempty(obj.J_k_accumarray_mat) % HACK!!!!!!
                obj.J_k_accumarray_mat = sparse(J_k, 1:length(J_k), 1, max(J_k), length(J_k));
            end
            G = obj.J_k_accumarray_mat*V_k;
